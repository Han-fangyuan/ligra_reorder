...
vertexSubsetData<data> edgeMapSparse_no_filter(graph<vertex>& GA,
    ...
    const flags fl) 
{
	//cout<<"调用Sparse_no_filter函数"<<endl;
  using S = tuple<uintE, data>;
  long n = indices.n;
...
template <class data, class vertex, class VS, class F>
vertexSubsetData<data> edgeMapData(graph<vertex>& GA, VS &vs, F f,intT threshold = -1, const flags& fl=0) 
{
	//cout<<"edgeMapData"<<endl;
	...
	//设置一个degrees数组和frontierVertices数组
	vs.toSparse();
	...
	{parallel_for (size_t i=0; i < m; i++) 
	{
		uintE v_id = vs.vtx(i);
		vertex v = G[v_id];
		degrees[i] = v.getOutDegree();//得到出度
		frontierVertices[i] = v;	//目前边界数组
	}}
...
//典型的edgemap，没有额外的数据被存储在每个顶点
//Regular edgeMap, where no extra data is stored per vertex.
//edgeMap(GA, Frontier, BFS_F(Parents))
...
//Note: this is the version of vertexMap in which only a subset of the input vertexSubset is returned
template <class F>
...
int parallel_main(int argc, char* argv[]) 
{
	...
	if (symmetric)//对称
	{
	  ...
	  graph<symmetricVertex> G = readGraph<symmetricVertex>(iFile,symmetric); //symmetric graph
	...
	{
		graph<asymmetricVertex> G = readGraph<asymmetricVertex>(iFile,symmetric); //asymmetric graph
		//cout << "图顶点数 : " <<G.n<<endl;
		//cout << "图边数 : " <<G.m<<endl;
		//indicator(G); 
		//cout << "readGraph<asymmetricVertex>"<<endl;
		Compute(G,P);
