...
/*
namespace decode_uncompressed : 各种函数
asymmetricVertex结构体【有向】
struct symmetricVertex  【无向】
...
{
	//1.
	//edgeMapDense使用，调用者确保每个顶点，满足条件cond(v_id),对于每个顶点，解码其入边
	//检查其邻居是否在当前frontier，在调用update函数更新，
	//按顺序处理边，一旦!cond(v_id)停止处理
	...
	{
		uintE d = v->getInDegree();//顶点入度
		if (!parallel || d < 1000) //非并行处理
		{
			for (size_t j=0; j<d; j++) 
			{
				uintE ngh = v->getInNeighbor(j);//入邻居
	...
	//2.
	//edgemapdenseForward函数,对于每个满足cond函数的出顶点，原子更新
