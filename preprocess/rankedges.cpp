...
//vgr —— csr格式边没有权重
//-->binary csr format with no weight for edges
//cvgr —— csr格式，边没有权重没有多余的边
// binary csr format with no weight for edges (no self or redundant edges)/
//csvgr —— csr格式，对称，边两个
//--> binary csr format with no weight for edges (no self or redundant edges)
//graph is symmetric -- so for every edge (u, v) there also exist (v, u)
//cintgr —— csr格式，有权重的
//--> binary csr format with int weight for edges (no slef or redundant edges)
...
int write_output_file(const string& output_format, const string& output_file, const vector<edge_t>& edges, const uint64_t& num_vert, const uint64_t& num_edges) 
{
    ...
    */
    ...
    else if ( output_format == "cintgr" ) //cintgr
    {
        vector<edge_t> cleaned_up_edges;
        status = make_edges_clean(edges, cleaned_up_edges, num_vert, num_edges);
        if ( status != 0 ) {
    ...
    if ( status != 0  ) 
    {
        cout << "fail to write to " << output_file << "." << endl;
    }
...
int main(int argc, char* argv[]) 
{
  ...
  h.print_c(argv[5]);
    ...
    string file = string(argv[5]);
    
    file =file.erase(file.size() - 3, 3);
